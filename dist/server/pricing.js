...
function calculateOrderPricing(items) {
    ...
    const grossTotalKobo = items.reduce((sum, item) => sum + item.unitPriceKobo * item.quantity, 0);
    // Step 2: Platform Service Fee — 1.5%, rounded to nearest Kobo
    const platformFeeKobo = Math.round(grossTotalKobo * exports.PLATFORM_FEE_RATE);
 ...
 */
function generateOrderRef(state, sequenceNum) {
 ...
 */
function buildMonnifyPayload(pricing, orderRef, merchantSubAccountCode, customerName, customerEmail, paymentMethod) {
    ...
    const merchantSplitPercentage = parseFloat(((pricing.merchantNetKobo / pricing.grossTotalKobo) * 100).toFixed(4));
